...
class DeepgramAgentManager:
    ...
    async def start(self) -> bool:
        """Connect, send Settings, wait for SettingsApplied. Retries on transient failures."""
        backoffs = [1, 2, 4]
        attempt = 0
        while attempt <= self._max_retries:
            try:
                await self._connect()
                # Measure Settings -> SettingsApplied latency
                loop = asyncio.get_running_loop()
                t0 = loop.time()
                await self._send_settings()
                ok = await self._wait_for_settings_applied(timeout=self._connection_timeout)
                if not ok:
                    raise TimeoutError("SettingsApplied timeout")
    ...
    async def apply_settings(self, new_settings: AgentSettings) -> bool:
        """Dynamically apply new settings and wait for SettingsApplied."""
        if not self._ws:
            raise RuntimeError("WebSocket not connected")
    ...
    def _build_settings_message(self, s: AgentSettings) -> Dict[str, Any]:
        return {
            ...
            "agent": {
                "language": s.language,
                "listen": {
                    "provider": {"type": "deepgram", "model": s.listen_model}
                },
    ...
    def _think_provider_block(self, s: AgentSettings) -> Dict[str, Any]:
        ...
        def _attach_endpoint(provider_block: Dict[str, Any]) -> Dict[str, Any]:
            # Attach optional endpoint and headers when configured (used for Google and custom endpoints)
            if LLM_ENDPOINT_URL:
                provider_block["provider"]["endpoint"] = LLM_ENDPOINT_URL
            return provider_block

        if t.startswith("open") and OPENAI_API_KEY:
            # Deepgram expects provider type "openai"
            return {"provider": {"type": "openai", "model": model, "temperature": temp}}
        if t.startswith("anth") and ANTHROPIC_API_KEY:
            return {"provider": {"type": "anthropic", "model": model, "temperature": temp}}
        if t.startswith("google") and GEMINI_API_KEY:
            # For Google/Gemini, include endpoint/headers when provided via config
            return _attach_endpoint({"provider": {"type": "google", "model": model, "temperature": temp}})

        # Fallbacks
        if OPENAI_API_KEY:
            logger.warning("Falling back to OpenAI for think provider")
            return {"provider": {"type": "openai", "model": "gpt-4o-mini", "temperature": 0.7}}
