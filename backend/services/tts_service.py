...
class TTSService:
    ...
    async def generate_speech_stream(self, text: str, speed: float = 1.0) -> AsyncGenerator[bytes, None]:
        ...
        try:
            ...
            def synthesize_sync():
                ...
                # Use wave module to create a wave.Wave_write object as required by PiperVoice.synthesize
                with wave.open(wav_buffer, 'wb') as wav_file:
                    wav_file.setnchannels(1)  # mono
                    wav_file.setsampwidth(2)  # 16-bit
                    wav_file.setframerate(self.sample_rate)
                    
                    # Piper's synthesize method writes directly to the wav_file
            ...
            audio_data_wav: bytes = await asyncio.to_thread(synthesize_sync)
    ...
    async def synthesize_speech(self, text: str, speed: float = 1.0) -> Dict[str, Any]:
        ...
        try:
            ...
            # Estimate duration based on WAV file size
            # For 16-bit mono audio, each sample is 2 bytes
            samples = (len(audio_data) - 44) // 2  # Subtract WAV header size (typically 44 bytes)
